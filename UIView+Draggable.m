...
- (void)handlePan:(UIPanGestureRecognizer*)sender {
    ...
    CGFloat fwidth = CGRectGetWidth(self.frame);
    CGFloat fheight = CGRectGetHeight(self.frame);
    if([classn isEqualToString:@"JoystickController"])
    {
        fwidth = 80.0;
        fheight = 80.0;
    }
    else if ([classn isEqualToString:@"DWBubbleMenuButton"])
    {
        fwidth = 40.0;
        fheight = 40.0;
    }
    ...
    CGFloat x1 = CGRectGetMinX(self.frame);
    CGFloat x2 = (([self shouldMoveAlongX]) ? translation.x : 0);
    CGFloat newXOrigin = x1 + x2;
    
    CGFloat y1 = CGRectGetMinY(self.frame);
    CGFloat y2 = (([self shouldMoveAlongY]) ? translation.y : 0);
    CGFloat newYOrigin = y1 + y2;
    ...
    if (!CGRectEqualToRect(cagingArea, CGRectZero)) {
        ...
        if (newXOrigin <= cagingAreaOriginX ||
            newXOrigin + fwidth >= cagingAreaRightSide) {
            // Don't move
        ...
        if(newYOrigin <= cagingAreaOriginY ||
           newYOrigin + fheight >= cagingAreaBottomSide) {
            // Don't move
    ...
    if([classn isEqualToString:@"JoystickController"])
    {
        ...
        if(newXOrigin <= 10 || newXOrigin >= KS_Width-10)
        {
            newXOrigin = KS_Width-125;
        }
        
        if(newYOrigin <= 64 || newYOrigin >= KS_Heigth-64)
        {
            newYOrigin = KS_Heigth-225;
        }
    }
